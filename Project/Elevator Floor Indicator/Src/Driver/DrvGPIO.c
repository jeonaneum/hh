...
/*              E_DRVGPIO_ARGUMENT								Incorrect argument                         */
/* Description:                                                                                            */
/*              Set the specified GPIO pin to invert.               			                                   */
/*---------------------------------------------------------------------------------------------------------*/
int32_t DrvGPIO_InvBit(E_DRVGPIO_PORT port, int32_t i32Bit)
{
	GPIO_T * tGPIO;
	volatile uint32_t u32Reg;
...
//    }
	...
	u32Reg = 1 << i32Bit;
	if(tGPIO->DOUT & u32Reg)	tGPIO->DOUT &= ~u32Reg;
	else	tGPIO->DOUT |= u32Reg;
...
{  
    ...
    switch ( function )
	{
		...
		case E_FUNC_SPI1:
		{
          	outpw(&SYS->GPCMFP, inpw(&SYS->GPCMFP) | (0x7<<9));
		  	outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0x1<<11));
		}break;
		...
		case E_FUNC_UART0_RX_TX:
			outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | 0x3);
	  	    //outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0xd<<11));
		break;
		case E_FUNC_UART0_RTS:
            outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | 0x7);
	  	    outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0x1<<11));
	  	break;
		case E_FUNC_UART0_RTS_CTS:
            outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | 0xf);
	  	    outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0x1<<11));
	  	break;
		case E_FUNC_UART1:
		case E_FUNC_UART1_RX_TX:
			outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | (0x3<<4));
	  	    //outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0x1<<11));
		break;
		case E_FUNC_UART1_RTS:
            outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | (0x7<<4));
	  	    outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0x1<<11));
	  	break;
